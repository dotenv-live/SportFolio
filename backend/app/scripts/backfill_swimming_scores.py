"""
Backfill script – recompute per-race scores & price history for swimming players.

For each swimming player, walks through races chronologically and at each race:
    1. Computes C, G, F, A, AI, PS using all races up to (and including) that point
    2. Stores the sub-scores on the ``player_matches`` document
    3. Derives price via FV → DI → smooth → price_history snapshot

Usage:
    cd backend/
    .venv/bin/python -m app.scripts.backfill_swimming_scores
"""
...
from app.services.swimming_performance import (
    compute_ai_score,
    compute_actual,
    compute_consistency,
    compute_fitness,
...
logger = logging.getLogger("backfill_swimming")
...
async def backfill() -> None:
    ...
    # ── Find all swimming players (case-insensitive) ─────────────────
    swimming_re = re.compile(r"^swimming$", re.IGNORECASE)
    players = []
    async for p in db.players.find({"sport": swimming_re}):
        players.append(p)

    if not players:
        logger.warning("No swimming players found – nothing to backfill.")
        client.close()
        return

    logger.info("Found %d swimming player(s) to backfill.", len(players))

    # ── Fetch sport config for AI weights / phi / metrics ────────────
    sport_config = await db.sports.find_one({"name": swimming_re})
    ...
    for player in players:
        ...
        # ── Gather all races, oldest-first ────────────────────────────
        races_asc: list[dict] = []
        async for m in db.player_matches.find({"player_id": pid}).sort("date", 1):
            races_asc.append(m)

        if not races_asc:
            logger.info("  %s: 0 races – skipping.", pname)
            continue

        # ── Delete existing price_history for this player (we rebuild it) ─
        del_result = await db.price_history.delete_many({"player_id": pid})
        logger.info(
            "  %s: cleared %d old price_history rows, processing %d races …",
            pname, del_result.deleted_count, len(races_asc),
        )

        prev_price = base_value  # for EMA smoothing
        history_docs = []

        for idx in range(len(races_asc)):
            # races up to this point, most-recent-first (as the engine expects)
            window = list(reversed(races_asc[: idx + 1]))
            current_race = races_asc[idx]
            ...
            ai = compute_ai_score("Swimming", ai_weights, window)
            ...
            # ── Stamp scores onto the race document ──────────────────
            await db.player_matches.update_one(
                {"_id": current_race["_id"]},
                {
                    "$set": {
                        "scores": {
            ...
            if race_date_str:
                try:
                    ts = datetime.strptime(race_date_str, "%Y-%m-%d").replace(
                        tzinfo=timezone.utc
                    )
        ...
        if last:
            latest_window = list(reversed(races_asc))
            ...
            ai_final = compute_ai_score("Swimming", ai_weights, latest_window)
        ...
        logger.info(
            "  %s: %d races scored, %d price_history rows, latest price=%.2f",
            pname, len(races_asc), len(history_docs),
            history_docs[-1]["price"] if history_docs else 0,
        )

    logger.info(
        "Backfill complete: %d races updated, %d price_history rows inserted.",
        total_races_updated, total_history,
    )
    client.close()
